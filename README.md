...
| **CSS** | Flexbox, CSS Grid Auto-Fit, Absolute/Flex Centering, Positioning & Z-Index, Box Model, Typography, Gradients, 3D Transforms, Keyframe Animations, Glassmorphism Filters, Container Queries, CSS Variables | `display: flex`, `grid-template-columns: repeat(auto-fit, minmax(...))`, `place-items: center`, `position: sticky`, `backdrop-filter: blur()`, `clamp()`, `:has()`, `container-type` |
